...
- [x] P5.1 Remove the `---reply---` parsing contract, delete only the obsolete reply-marker helper/tests, and post the complete final model message. Preserve P1's separate stop helper. Update Slack wording instructions and ENG-13251-related prose. Tests: no `reply---` remains under `agent/`, Slack channel discovery, `pnpm validate`. done 2026-08-29T23:18Z, the marker helper and its tests are deleted, `message.completed` posts the complete final message verbatim with blankness alone deciding the typing fallback, the wording skill and channel docs describe the verbatim post, and a scan test pins the marker's absence under agent/.
- [x] P5.2 Add `agent/lib/slack-post.ts` with a 12,000-character chunker that prefers paragraph then line boundaries and hard-cuts only when needed. Route final replies through it. If posting fails, log the original failure and attempt exactly one short fallback naming the Slack error. Tests: short reply, paragraph/line/hard splits, content preservation, empty input, ordered posting, and rejected-post fallback; `pnpm validate`. done 2026-08-30T00:28Z, splitSlackReply cuts at paragraph/line boundaries with a surrogate-pair back-off on hard cuts and postSlackReply posts ordered chunks then one bounded fallback that names only an allowlisted Slack error code, wired into message.completed with 14 focused tests.
...
- [x] P6.1 Rewrite the root triage/reporting/handoff contract for one critic delegation, one progress line, and one adjudication. Preserve the urgent-human hotlane. Put blocking findings in the ticket document's Review line. Update triage skill tests to reject attempt/re-delegation wording. Tests: focused triage tests, `pnpm validate`. done 2026-08-30T01:25Z, the critic now runs exactly once per ticket with one progress line and one adjudication, CHALLENGE/INSUFFICIENT_EVIDENCE/review failure are adjudicated against the Stage 4 record and routing continues with findings named in an Adjudicated Review line keyed by the Stage 5 read-back updatedAt, and the hotlane stop is unchanged.
- [x] P6.2 Remove attempt numbering and retry language from the critic's agent description, instructions, child skill, and tests. Keep the critic read-only and evidence-bound. Tests: critic and critic-capabilities suites, `pnpm validate`. done 2026-08-30T02:12Z, the critic packet no longer takes an attempt number, the Attempts section is now a One review statement matching P6.1's exactly-once contract, and a case-insensitive phrase scan pins the removal across description, instructions, and skill.
...
- [x] P3.1 Move Stages 5 through 7 and report templates verbatim into a new `triage-handling` skill, update links and skill roster, and keep tests reading both halves. This is a move-only checkpoint before editing prose. Tests: triage and Slack-intake suites, `eve info`, `pnpm validate`. done 2026-08-30T13:12Z, Stages 5-7 plus Follow-ups and Reporting reference moved verbatim into the new discovered `triage-handling` skill with `references/reporting.md`, the intake skill keeps Stages 1-4 and gains a Handoff to handling section, engineering-handoff roster links now name `triage-handling`, and the intake description advertises only Stages 1-4 with the handoff pinned by tests.
- [x] P3.2 Fold the triage tools reference into a compact exact tool-name table in the intake skill and delete the redundant reference file. Pin the full catalog in tests. Tests: triage skill tests, `pnpm validate`. done 2026-08-30T13:37Z, the 19.3 KB tools.md folded into a 20-row exact tool-name table plus compact trap notes in the intake Stage 4 lane section, the reference file is deleted, and tests pin the catalog by parsing each Markdown row and deep-equaling surface, call prefix, and ordered token set.
- [x] P3.3 Trim the intake skill's introduction, rules, and Stages 1 through 4 to 15,000 bytes or less. Preserve all evidence lanes, memory rules, trust boundaries, project-independent retrieval, and the explicit handoff to `triage-handling`. Tests: relevant triage cases, byte check, `pnpm validate`. done 2026-08-30T17:29Z, 19,809 to 14,967 bytes: per Aaron's resolution of the 14:50Z blocker, the exact tool catalog plus its call and trap notes (5,153 bytes) moved verbatim into `agent/skills/triage-investigate/references/tools.md`, gated by a mandatory Stage 4 subsection that names the reference and forbids continuing into evidence collection until it is loaded. Every evidence lane, memory rule, trust boundary, project-independent retrieval rule, Stage 1-3 mechanic, repository behavior, and the handoff to `triage-handling` stays in the intake skill. Tests read both files, deep-pin the catalog by row parsing in the reference, assert the reference path and mandatory load timing ahead of the first evidence lane, keep the three restored regression pins (complete Linear field list, conditional Sentry `search_issues`, bounded COUNT/SELECT), and add the permanent 15,000-byte intake budget check.
- [x] P3.4 Trim handling Stage 5 to roughly 9 KB without changing verdict, unblock, priority, labeling, critic, or routing semantics. Tests: all Stage 5 contract cases, `pnpm validate`. done 2026-08-30T15:32Z, Stage 5 trimmed 13,674 to 8,261 bytes: prose tightened with every pinned contract kept, and the one-pass critic protocol moved verbatim into `references/critic-review.md` (loaded only for a non-duplicate Bug) behind the exactly-once gate and pointer left in Stage 5, with contract tests now reading both halves.
...
- [ ] P3.6 Add the permanent byte-budget test for the handling skill (the intake skill's 15,000-byte check landed with P3.3) and record the measured before/after totals in the session log. Tests: full `pnpm validate`.
...
- 2026-08-29T23:18Z P5.1: validate pass (387 tests, 0 errors/warnings); Codex review rounds 1 (REJECT: 1 high — the post trimmed data.message before posting, breaking the verbatim contract; fixed by gating the typing fallback on trim but posting the original message, plus a whitespace-preservation assertion; final validate pass; second round not run because the fix changed one post call, not the design). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T00:28Z P5.2: validate pass (402 tests, 0 errors/warnings); Codex review rounds 2 (round 1 REJECT: 1 blocking — hard cut could bisect a UTF-16 surrogate pair, 1 medium — arbitrary exception text in the visible fallback, 1 low — uncaptured console.error in tests; all three fixed with a surrogate back-off, an allowlisted Slack error code with generic fallback, and captured logs. round 2 REJECT: 1 medium — unbounded error-code capture could overflow the fallback; fixed with an 80-character bound plus adversarial regression, final validate pass; third round not run per protocol cap). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T01:25Z P6.1: validate pass (402 tests, 0 errors/warnings); Codex review rounds 2 (round 1 REJECT: 3 blocking, 1 medium — adjudicated Review line carried no updatedAt for the handoff version check, findings sat under instead of in the Review line, handoff line 74 still demanded a new critic review, and the rejection scan covered only the Stage 5 excerpt; all four fixed with the read-back-updatedAt settled-version contract, inline finding clauses, adjudication wording, and an all-surface scan. round 2 APPROVE: 0 findings, final validate pass). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T02:12Z P6.2: validate pass (403 tests, 0 errors/warnings); Codex review rounds 1 (REJECT: 1 high — the One review sentence still said "no re-review and no retry", 1 medium — the test scan was case-sensitive and missed surviving phrases; both fixed with a positive single-adjudication sentence and a lowercased 13-phrase scan, final validate pass; second round not run because the fix changed one sentence and one test, not the design). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T13:12Z P3.1: validate pass (404 tests, 0 errors/warnings); Codex review rounds 1 (REJECT: 1 high — intake skill description still advertised the moved handling responsibilities, 1 low — reporting.md still named triage-investigate as the Stage 6 owner; both fixed with an intake description scoped to Stages 1-4 plus the handoff pinned by new description assertions, and a one-line owner rename; final validate pass; second round not run because the fixes changed description metadata and tests, not the design). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T13:37Z P3.2: validate pass (404 tests, 0 errors/warnings, exit 0 with connector env sourced); Codex review rounds 1 (REJECT: 1 blocking — the worker's own eve info run lacked LINEAR_CONNECTOR, disproven by the coordinator's clean env-sourced validate, 2 high — loose substring catalog pin and discarded Intercom/Instantly call semantics, 1 medium folded into the same lane fix; both highs fixed with a row-parsing deep-equal catalog pin and concise URL-or-email call order, raw contact-id prefix stripping, structured-versus-text search distinction, and accepted-only/cap-failure rules restored to the lane prose with focused pins, final validate pass; second round not run because the fixes strengthened one test and restored prior lane semantics, not the design). Worker dispatch initially failed at agent_readiness while Codex was still starting; retried with --retry-of on the same terminal after tui-idle.
- 2026-08-30T14:50Z P3.3 BLOCKED: validate pass (404 tests, 0 errors/warnings, exit 0 with connector env sourced); Codex review rounds 1 (REJECT: 1 high — full Linear field list trimmed from the Stage 1 read, 2 medium — Sentry `search_issues` made unconditional and the bounded COUNT/SELECT guardrail dropped; all three restored and pinned with three new test assertions, final validate pass; second round not run because the fixes restored prior sentences, not the design). Blocker: byte floor measured at 19,809 vs the 15,000 target; the pinned tool catalog (3,678 bytes) plus preserved lane/memory/trust rules exceed the budget, so the item needs Aaron to relax the budget, authorize behavior deletion, or move the catalog (reverses P3.2). Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T15:32Z P3.4: validate pass (404 tests, 0 errors/warnings, exit 0 with connector env sourced); Codex review rounds 1 (APPROVE: 0 blocking, 0 high, 0 medium, 0 low). Stage 5 measured 13,674 bytes before and 8,261 after: pure prose trim alone floored near 12.7 KB, so the pinned one-pass critic protocol (about 5.7 KB) moved verbatim into references/critic-review.md behind the gate and pointer in Stage 5, matching the P3.1 split pattern. Same Codex hooks trust prompt as P4.2; trusted and retried with --retry-of on the same terminal.
- 2026-08-30T17:29Z P3.3: validate pass (406 tests, 0 errors/warnings, exit 0 with connector env sourced). Resolved the 14:50Z blocker with the agreed design: the 5,153-byte exact tool catalog and its trap notes moved verbatim into references/tools.md behind a mandatory Stage 4 pre-evidence load gate; intake skill measured 19,809 bytes before and 14,967 after. Tests read both files, deep-pin the catalog by row parsing in the reference, assert the reference link and load timing before the first evidence lane, keep the three regression pins from the blocked round, and add the permanent 15,000-byte intake budget check; the handling-skill budget test stays with P3.6.
