...
- [!] P3.3 Trim the intake skill's introduction, rules, and Stages 1 through 4 to 15,000 bytes or less. Preserve all evidence lanes, memory rules, trust boundaries, project-independent retrieval, and the explicit handoff to `triage-handling`. Tests: relevant triage cases, byte check, `pnpm validate`. blocked 2026-08-30T14:50Z: trimmed 26,812 to 19,809 bytes with all pinned contracts green, but 15,000 is unreachable without dropping preserved content. The P3.2 tool catalog is 3,678 pinned bytes and the lane, memory, trust-boundary, and project-independence rules account for most of the rest; Codex already flagged three behavior drops at this size (restored and pinned), so further cuts eat guardrails the item says to keep. Remaining work: Aaron picks one of relaxing the budget to about 19,800 bytes, deleting Stage 1-3 mechanics (money-ask redirect, vision routing, claim gate, identity branches), or moving the exact tool catalog back out of the pre-evidence load, which reverses P3.2.
