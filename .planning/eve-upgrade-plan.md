# Foreman: remove the factory, then upgrade Eve

Approved by Aaron on 2026-09-11. Master: ENG-13735. Execute one PR at a time; Aaron tests the first before the next opens. Do not merge without Aaron's explicit approval.

## Outcome and sequence

Foreman remains a general-purpose Eve agent with active integrations, support schedules, investigation memory, critic, vision, and direct repository work. Remove all unused factory orchestration. GitHub starts work only for explicit trusted mentions. Restore Eve's native agent delegation. Slack ask_question stays disabled; default GitHub write approvals and input-token budget pauses stay disabled.

1. Disposable upgrade spike, 0.5 day: completed against the original checkout; restored package.json, lockfile and Eve 0.44.0. Exact outputs are in the local ignored .context/eve-upgrade-spike directory.
2. PR1, remove factory on Eve 0.44.0: 2.5 days including Preview setup and real smoke.
3. PR2, upgrade simplified Foreman to Eve 0.54.2: 2.5 days including Preview setup and real smoke.
4. Contingency: 0.5 day. Total: 6 working days, excluding upstream browser release and Aaron's review/testing waits.
