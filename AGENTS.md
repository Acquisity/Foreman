...
- The `factory-pipeline` skill is lane-scoped by `agent/lib/factory-lane.ts`. An unattended run gets the same `PIPELINE` text inline in `FACTORY_PROMPT` and is offered no duplicate skill; an intake-only Slack channel needs explicit trusted factory intent; every other session needs explicit factory intent or a selected repository. Intent is read from the delivered message by the dispatching channel and stamped on the auth, because a dynamic skill resolver runs at `turn.started` where eve hands it an empty `ctx.messages`. A bare `owner/repo` token in free text stamps no repository, so it grants no factory skill either. The direct general path is unchanged.
