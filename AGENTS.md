...
- Intercom is deliberately app-scoped. Its private Acquisity workspace access token lives in a Vercel Connect API-key connector while requests still go to Intercom's MCP endpoint. It has no caller consent path. An authorization failure is deployment or operator configuration, not a reason to ask a Slack requester to sign in or to retry the same call. Never restore OAuth or article tools without revisiting this trust and scope design.
