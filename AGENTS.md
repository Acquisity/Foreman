...
- Slack channel IDs listed in `SLACK_INTAKE_ONLY_CHANNELS` are intake-only: conversation runs as normal, but the session is stamped intake-only and `intakeOnlyPolicy` denies `prepare_repository`, so engineering work is filed to Linear instead of started there.
