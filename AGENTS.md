...
- Foreman's running costs come from the fixed `read_foreman_costs` tool, never from the Vercel MCP connection, which has no usage or billing tool. AI Gateway spend and credits go through the `ai` package's gateway provider on the deployment's own OIDC token. Vercel platform usage comes from the FOCUS billing export, `GET /v1/billing/charges`, which the OIDC token cannot read, so it uses the app-scoped API-key connector named by `VERCEL_API_CONNECTOR`, holding a team access token used for that one GET. The variable is optional and read at call time: unset drops the platform line from the report instead of failing the build. The `foreman-costs-report` schedule posts the tool's rendered `report` unchanged.
