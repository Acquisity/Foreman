...
- **Slack** — mentions and DMs trusted by channel membership. Channels listed as intake-only can investigate and answer but cannot ship code. An ask filed in the mismatched intake channel is handled in place: the reply notes which kind of ask it is and the matching triage procedure runs there, with the project and labels routed from the evidence rather than the channel. A screenshot or file attached to a mention is staged by eve under `/workspace/attachments` in the sandbox, and the Slack channel adds one context line naming the files and that directory, so the text-only chat model can hand the path to the vision subagent on the same turn. Later mentions queue behind a running turn and get one line saying so. A turn still running posts one progress line every 5 minutes until it ends, with the status echo restored after each line; a line comes due at the next lifecycle event, so factory stations are asked to return within a few minutes of work so that event arrives. A message that is just `stop` or `cancel` cancels the running turn; one origin-neutral cancellation notice posts when eve's cooperative cancellation ends the turn at its next step boundary, the same line for a stop, a reset, or a declined session limit, and the closing post of a factory turn carries the pull request link.
