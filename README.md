...
`agent/hooks/ops.ts` writes one bounded JSON line per lifecycle event through `logOpsEvent` in `agent/lib/ops-log.ts`, so `vercel logs` can be counted. The eleven events are `session.started`, `session.completed`, `session.failed`, `turn.started`, `turn.completed`, `turn.cancelled`, `turn.failed`, `step.failed`, `authorization.required`, `input.requested`, and `action.result`. `action.result` covers every tool call, not only the failures, so a scan can tabulate reach and failure per tool and per connection: it names the tool, the connection derived from the `<connection>__<tool>` shape or none for an authored root tool, `ok` or `error`, the session id, and the turn id. It logs no error code, because eve derives that code from the tool's own output. It carries no duration, because neither the request nor the result event carries a timestamp and the hook keeps no state. No line ever contains a tool input, a tool output, a message body, or customer data.
