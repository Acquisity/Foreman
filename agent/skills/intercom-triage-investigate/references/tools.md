...
Call `list_instantly_subworkspaces` first. It follows every Workspace Group page, returns only accepted subworkspaces, reports pending/rejected counts without exposing those workspaces, and includes the admin workspace name and ID. Prefer the selected subworkspace's ID. Name selection in `read_instantly_subworkspace` is normalized but must resolve exactly once; zero, ambiguous, pending, and rejected matches fail closed.

`read_instantly_subworkspace` accepts `accounts`, `campaigns`, or `emails`, adds `x-as-workspace` server-side only after validating the selection, and returns the workspace name and ID on every page. Pass `nextStartingAfter` back as `startingAfter` until it is null. Email reads always use preview-only mode and remove message bodies, attachment payloads, and raw address arrays. Short 429 and transient failures receive bounded retries; a longer rate limit returns its safe retry interval. There is no generic path, method, header, or body input and no mutation route.
