...
Both authored helpers validate the complete accepted membership set. If the complete list exceeds its 256 KiB output limit, pass a provider workspace ID or exact provider name established by current evidence directly to `read_instantly_subworkspace`; it does not skip membership validation. A 100-page cap means membership is incomplete and the lane could not run. An unmatched name proves only that the selector did not resolve. It is not evidence that the customer has no provider workspace or that provisioning failed. Look for the linked provider ID in the pinned production scope; if it remains unknown, mark provider resource state unverified. A matched, successful resource read may return valid empty results.
