...
`create_triage_review_packet`, `search_investigation_memory`, `read_triage_review_verdict`, `record_investigation_case`, `correct_investigation_case`, `reserve_triage_master`, and `complete_triage_master_reservation` are authored tools in `agent/tools/`. They are the whole interface to immutable packet creation, investigation memory, critic attestation metadata, and the causal master reservation. Never list database projects, inspect schemas, roles, indexes, or rows, test SQL, look for credentials, or otherwise hunt for a backing database. If one of these tools answers, the wiring is working. If it returns `available: false` or a write fails, record that internally when relevant and continue the investigation without trying another database connection. A missing critic attestation blocks Bug finalization, and a reservation failure blocks new-master creation.

`search_investigation_memory` has two retrieval modes. The existing Linear-first triage procedure passes the ticket's Linear project id, which picks one product area. The Intercom-native product procedure may omit `linearProjectId` before a ticket exists; that authorized call searches the server-owned list of six live product areas, excludes planned Acquisity Agent cases, identifies every result's area, and returns wider-incident signals separately per area. It does not let the model supply an area list.

Pass `sourceIssueId` alongside a Linear project id to look up one ticket's own case, which is how you find the case id to correct. That identity form still requires the project and ignores relevance filters and the time window. An unmapped project, an unreachable store, and an unconfigured store all return `available: false`; none blocks the investigation. Results are capped at 10, default 5.

`read_triage_review_verdict` reads the server-attested result of an actual completed critic call. Only `APPROVE` returns an opaque `approvalId`. Pass that id as `criticApprovalId`; never construct approval fields from the critic's prose or structured output.

`record_investigation_case` runs once, after the Triage investigation document and final external writes are complete. For a Bug, final means `triage-critic` approved the exact current evidence revision and `read_triage_review_verdict` returned its opaque approval id. The record tool verifies that attestation and the reviewed claim, cause, confidence, model, packet, and repository revision. It refuses payloads carrying email addresses, organization or user ids, connection strings, or credential-shaped tokens, so keep those in the document. A second write for the same ticket is refused: a changed conclusion goes through `correct_investigation_case`, which takes the full corrected case plus the active case id and requires a new attested approval when the correction is a Bug. Keep the `caseId` the original write returned. A later session will not have it, so pass `sourceIssueId` to `search_investigation_memory` to get it back. That is an identity lookup rather than a search: it ignores the relevance filters and the time window, so neither ranking nor a case older than a year can hide it. The result cap still applies, which costs nothing here because a ticket has exactly one active case.
