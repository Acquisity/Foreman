...
2. Call `route_ticket` on this ticket once with the `decide_triage` `route` unchanged, plus `parent` and `inheritAssigneeFrom` both set to the master and `assignee` set to `route.assignee`, or when it has none the area owner from the area-routing roster in `triage-handling` Stage 6, so the report carries its own decisions and the child never sits under a master owned by someone else. The tool inherits the master's assignee when it has one and uses the area owner only when it does not; say in the document when the master was unassigned.
