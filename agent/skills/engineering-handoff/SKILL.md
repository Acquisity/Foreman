...
3. Match the complete eligible candidate set by cause. If an existing master owns the cause, read both issues and their relations first. Parent the source only when that parent is absent, append a related link only when that exact relation is absent, and link the source report from the master only when that exact link is absent. Add only aggregate evidence, recount blast radius, inherit its owner, and apply the exact critic-approved priority and hotlane state while preserving existing valid labels. Never lower severity because of a workaround or one new report. If the final priority, hotlane state, or planned write differs from the approved packet, stop for the remaining review attempt or route to a person. Do not reserve or create. Treat an exact existing relation as successful idempotent state, not a reason to append it again.
4. If no eligible existing master owns the cause, call `reserve_triage_master` with the opaque `criticApprovalId` and source issue id. The server reads stable causal identity keys and the reviewed master generation from the attested packet. A normal first master uses the initial generation. A 30-day workflow replacing a reviewed stale same-cause master uses that stale master id as the next generation. Only the transaction that atomically reserves that exact causal generation receives `acquired: true`; every conflict or replay fails closed. When it returns `existingMasterIssueId` and `existingMasterCreatedAt`, apply the active workflow's recency rule before reuse. When it returns `reviewed_generation_changed`, use the remaining review attempt for the changed candidate or route to a person. When it returns `reservation_in_progress` or is unavailable, stop and reconcile against Linear or route to a person. An unresolved reservation never expires into permission for another create.
5. Only the caller that receives `acquired: true` may create exactly one master. Create it with the exact critic-approved priority and hotlane state while preserving every valid required label, and keep its `reservationId`.
6. Parent the source report once and preserve existing valid labels as a union when the API replaces labels.
7. Read back the master, source report, parent relation, labels, assignee, project, priority, hotlane state, and links.
8. After a newly created master and the source's parent relation pass readback, call `complete_triage_master_reservation` with the reservation id, master issue id, and source issue id. The tool independently reads the master, verifies that the exact source is parented to it, reads the master `createdAt`, and binds it only when the source and time match the active reservation. If completion is ambiguous, retain the created issue identifier and route the mismatch to a person. Never create another master.
