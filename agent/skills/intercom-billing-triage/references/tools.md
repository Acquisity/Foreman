...
Root tools are authored in `agent/tools/` or provided by the eve framework. They are called by their bare name with no prefix: `prepare_repository`, `grep`, `glob`, `read_file`, `bash`, `lookup_customer`, `read_billing_account`, `planetscale_execute_read_query`, `save_investigation_document`.
...
Read them in flow order: Intercom, then PlanetScale, then Autumn, then Stripe. Autumn and Stripe use app-scoped root tools in this intake workflow, not the requester's personal MCP grants; when one cannot run, its Executor read is the fallback.
