...
One match is enough. If the email belongs to multiple workspaces, pick the workspace the evidence establishes and use its membership `organizationId` as the pin (`pinnedOrganizationId` is null then), name the others in the document, and ask only when the choice changes the financial verdict and evidence cannot settle it. If no live workspace carries the email: run no customer-scoped read, do not call `decide_billing`, ask for the workspace or billing email (in an unattended run, report it once for Aaron instead), and end the turn. Never name-match in place of the email anchor.
