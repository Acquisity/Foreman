...
Never guess a tool name. A service's REST API, its CLI, and its MCP server rarely share naming, and an invented call fails in a way that reads like the customer has no data. When a tool you want is not listed here, read the connection's live tool list first. If you cannot, say so in the document rather than trying names until one sticks.
