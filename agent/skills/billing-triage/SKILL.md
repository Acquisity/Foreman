...
1. **PlanetScale**, first and always. Call `read_billing_account` with the `pinnedOrganizationId` from `lookup_customer`. It returns the organization with `partnerId`, the billing account with provider, subscription status and plan, and every wallet, plus the credit balance rows and the last 20 credit transactions and manual credits; a `truncated` flag names a history list that hit its cap. Read `organization.partnerGoverned` before anything else; when it is true the partner rule below applies. This is the workspace, which is what the customer actually sees, so it is where their account of events is grounded. Order and invoice rows beyond that still come from `planetscale_execute_read_query`, scoped to the same organization. This is the only database this skill reads.
2. **Autumn**, second. Call the root tool `read_autumn_billing` with `billingAccount.id` from `read_billing_account`. Acquisity keys Autumn customers by billing account id, so `pinnedOrganizationId` is the wrong key and answers `customer_not_found`. It returns the customer's subscriptions, expanded plans and add-ons, feature balances, and `stripe_id` through a fixed read route. If it answers `available: false` because the source could not run (not reached, HTTP 5xx, not authorized), record the gap and continue without retrying Autumn this turn. The helper and discovered tools use the same Executor access. When a successful customer read leaves a catalog question, discover Autumn `getPlan` or `listPlans` through Executor. These reads cannot change billing. A 404 or `customer_not_found` is an answer about the id, not about Autumn: re-resolve the id from `read_billing_account` before writing "unavailable", "no plan", or "unverified". The one expected 404 is a partner-governed organization, `organization.partnerGoverned` true, which is what puts an account on Whop; it has no customer in Acquisity's own Autumn, so that 404 is the partner rule, not an id problem. When `billingAccount` is null the organization has no billing account, which is a finding, not an Autumn outage.
3. **Stripe**, last. The Stripe customer id is the `stripe_id` on the Autumn record; PlanetScale does not store it, so Stripe cannot be read before Autumn. Call the root tool `read_stripe_billing`: use `customer` with that `cus_` id for bounded history; `charge`, `refund`, or `dispute` for a known Stripe object; `promotion_code` for a customer-facing code; or `coupon` for a known coupon id. If a customer section says `has_more: true`, that history is incomplete. Withhold the amount or refund verdict until an exact object id is identified and read. If it answers `available: false` because the source could not run, record the gap and continue without retrying Stripe this turn. The helper and discovered tools use the same Executor access. For evidence outside a successful helper result, discover Stripe `stripe_api_read`, `stripe_api_search`, or `stripe_api_details` through Executor. These reads cannot move money.

A helper error of `operation_bindings_missing`, `invalid_operation_bindings`, or `operation_not_bound` means local configuration rejected the call before a provider request. In that case, discover and try the matching approved Executor read once, and report the helper configuration gap. This exception does not apply to authorization, network, timeout, or provider failures.
...
Exact tool names, per-system traps, and vendor docs are in [references/tools.md](references/tools.md). Read it before composing a call. Discover company tools within the shared Executor toolkit, describe the returned path, and invoke that exact path; root tools such as `read_autumn_billing`, `read_stripe_billing`, `list_instantly_subworkspaces`, `read_instantly_subworkspace`, and `planetscale_execute_read_query` are called bare. Never invent a tool name from a service's REST API or CLI; an invented call fails in a way that looks like the customer has no data.
...
`read_billing_account` returns `organization.partnerId`, `organization.partnerGoverned`, and `billingAccount.provider`. Route on `partnerGoverned`, never on the provider alone and never on `partnerId` being non-null: native Acquisity organizations carry the default partner id `00000000-0000-0000-0000-000000000001`, which is not a partner. `partnerGoverned` true means a partner owns the account, whatever the provider says.
...
**Ask**: <taxonomy bucket>
**Provider**: <Autumn | Whop, per partnerGoverned>
