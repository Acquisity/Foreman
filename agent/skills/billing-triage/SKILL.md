...
3. Identity gate: call `lookup_customer` with the customer email and pin `pinnedOrganizationId` before any other lookup. If `ambiguous` is true, stop and ask which workspace. If `error` is set, the lookup could not run; say so rather than treating the customer as missing. If `found` is false, or `memberships` is empty, no live workspace carries that email: say so and stop rather than reading billing for a null organization.
