...
A bug with a `parentId` is one more customer hitting an issue whose root cause already lives on the master ticket. Read the master with linear `get_issue` on the `parentId` for its title and `url`, and count the reports hanging off that master with linear `list_issues` filtered to `parentId` set to the master, so the note can say how many reports there are in total.
