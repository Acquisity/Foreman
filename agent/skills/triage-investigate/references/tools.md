...
`critic` is a declared subagent, called through the subagent tool by name. It owns its output contract; never pass an `outputSchema`. The message carries the source issue id, the `Triage investigation` document id and `updatedAt`, the full 40-character commit Stage 4 read, the proposed decisions, and the hotlane route. Foreman delegates to it exactly once per ticket and adjudicates the single result; there is no second delegation. It returns `verdict` (`APPROVE`, `CHALLENGE`, `INSUFFICIENT_EVIDENCE`), `reviewed` (the identifiers it actually reviewed), twelve `criteria_results`, `blocking_findings`, `advisory_notes`, and `summary`. It has its own sandbox and read-only copies of the evidence connections; it can take a few minutes and cannot write anywhere.
