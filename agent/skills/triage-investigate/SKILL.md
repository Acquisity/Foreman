...
7. On `CHALLENGE`, on `INSUFFICIENT_EVIDENCE`, or on a failed review: adjudicate once against the Stage 4 evidence record. Take each blocking finding, or the gap the critic named, with its named next check: re-run the evidence lane or correct the decision it points at. When the review could not run at all, re-check the pivotal claims directly against the recorded lanes. When the corrected record no longer supports the classification, change the classification and handling path to what the evidence does support. First update only the relevant evidence and decision sections with the corrected facts, without adding settlement-summary clauses elsewhere in the document, save it, and read back its adjudicated-evidence `updatedAt`. Then save it once more with `**Review**: Adjudicated <that adjudicated-evidence updatedAt> at <commit>: <CHALLENGE | INSUFFICIENT_EVIDENCE | review failure>` followed by one short clause per blocking finding or the failure reason, each naming what settled it, and read back the final `updatedAt`. Those clauses occur exactly once, in the `**Review**` line. That final read-back `updatedAt` is the settled version the handoff checks, exactly as the critic's echo is on an approval; the earlier adjudicated-evidence `updatedAt` remains the audit reference in the `**Review**` line. Continue to Stage 6 with both values and the settled decisions. In a read-only run, carry the corrected text inline with the `read-only` markers instead.
