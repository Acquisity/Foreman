...
2. Load `incident-hotlane` and produce its assessment from the document. It changes nothing; its route, proposed label, workstreams, and blast-radius figures are inputs to the review and to Stage 6.
...
4. Read the result as a whole before acting on `verdict`. An `APPROVE` that carries any `FAIL` criterion, fewer than twelve `criteria_results`, or a non-empty `blocking_findings` is handled as a `CHALLENGE`. A result that does not match the child's contract, a delegation error, a timeout, or a child that could not load its skill is a stop, not a retry.
