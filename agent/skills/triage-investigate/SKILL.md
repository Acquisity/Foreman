...
Every decision above is provisional until the review below has approved the document that records it. Do not apply the state, priority, labels, or project to the ticket yet; Stage 6 does that, and only for an approved document version.

### Review a Bug before routing it

This review runs only when the classification is `Bug`. `User Error`, `Platform Limitation`, and the unproven stop go straight to Stage 6.

1. Write the investigation document now. Create or update the issue-scoped `Triage investigation` document exactly as Stage 6 describes, from the template in [references/reporting.md](references/reporting.md), with every Stage 4 lane, the Stage 5 decisions, and the line `**Review**: Pending critic`. Read the document back and keep its id and `updatedAt`; the critic reviews that exact version. A first `CHALLENGE` edits it for attempt 2 and a stop marks it `Stopped`; once a version is approved, nothing may change it until routing is done. When Aaron has asked for read-only validation, write nothing to Linear: pass the document id and `updatedAt` as the literal marker `read-only` and carry the full document text in the critic message, on every attempt.
2. Load `incident-hotlane` and produce its assessment from the document. It changes nothing; its route, proposed label, workstreams, and blast-radius figures are inputs to the review and to Stage 6. If the route is `NEEDS_HUMAN_URGENT`, do not call the critic: set the document's `**Review**` line to `Stopped: NEEDS_HUMAN_URGENT` with the assessment under it, assign Aaron Fraga with a one-line comment that a possible high-risk incident is awaiting confirmation, leave the ticket's state, priority, and labels untouched, and skip to Stage 7 with nothing settled; that reply states only what is known, says a person is confirming it, and promises no action. In a read-only run, describe those writes instead of making them.
3. Delegate to the `critic` subagent with exactly: the source issue id, the document id and its `updatedAt`, the repository commit Stage 4 read (the full 40-character SHA from the Code path section), the proposed decisions (classification, unblock, handling path, state, priority, labels, the hotlane route and proposed label, and any existing master candidate with the match rationale), and `attempt 1`. Name the two or three claims the decision actually turns on, so the critic verifies those first. Do not pass an `outputSchema`; the child owns its contract. Do not paste the document text unless this is a read-only run.
4. Check the echo before reading the verdict. If `reviewed.commit` is `unpinned` or is not exactly the full 40-character commit you supplied, or `reviewed` does not echo exactly the issue id, document id, and `updatedAt` you supplied (or the `read-only` markers you supplied), the critic's checkout or packet failed, not the evidence. Whatever the verdict says, do not count that result as an attempt: re-delegate once with the same attempt number and the same inputs. If the second result is also `unpinned` or mis-echoed, it is a stop.
5. Read the result as a whole before acting on `verdict`. An `APPROVE` counts only when `criteria_results` holds exactly one entry for each of the twelve criterion slugs with no slug missing or repeated, no criterion is `FAIL`, and `blocking_findings` is empty. An `APPROVE` with any `FAIL` criterion, a missing or repeated slug, or a non-empty `blocking_findings` is handled as a `CHALLENGE`. A result that does not match the child's contract, a delegation error, a timeout, or a child that could not load its skill is a stop, not a retry.
6. On `APPROVE`: keep the critic's echoed `document_updated_at` and `commit` and continue to Stage 6 with the decisions as approved. Do not touch the document: the handoff checks that the version Linear holds is the one the critic reviewed, and the `**Review**` line is written only after routing, as Stage 6 describes. Any change to the evidence or the decisions reopens the review.
7. On the first `CHALLENGE`: address each blocking finding with its named next check, once. Re-run the evidence lane or correct the decision it points at, update the document, read back its new `updatedAt` (in a read-only run, carry the corrected text inline with the `read-only` markers instead), and delegate again with `attempt 2`, the new `updatedAt`, and the list of criteria that failed. Attempt 2 re-judges those criteria against the corrected document. When the correction changed a material decision rather than a detail, say so, because the critic then re-judges all twelve.
8. On a second `CHALLENGE`, on `INSUFFICIENT_EVIDENCE`, or on any stop from steps 4 and 5: do not classify, route, label, or record. Set the document's `**Review**` line to `Stopped: <verdict or failure>` with the blocking findings copied under it, so the next person starts from the evidence rather than from nothing. Assign Aaron Fraga as the explicit human-routing fallback with a one-line comment saying the investigation is recorded in the document and needs a person to settle it, leave the ticket's current state, priority, and labels untouched, and skip to Stage 7 with nothing settled; that reply states only what is known, says a person is confirming it, and promises no action. In a read-only run, describe those writes instead of making them. The Stage 7 reply after a stop states only what is known, says a person is settling it before anything is final, and promises no action. There is no attempt 3 and no fresh reviewer chain.

The critic reads evidence and recommends; Foreman adjudicates. A challenge is not an insult to the investigation, and an approval is not permission to skip readback.

Completion: either the unproven branch has made the unblock explicit, preserved the current ticket state, documented the missing confirmation and reopen condition, and stopped before classification, engineering routing, or memory; or one evidence-backed classification and handling path exist, the unblock is explicit, the final Linear state, numeric priority, and complete label union are decided, and, for a `Bug`, the critic approved the exact document version that records them or the review stopped and the ticket is with a person.
...
Inputs: the Stage 5 decisions and the completed investigation record. For a `Bug`, the review in Stage 5 approved the document version those decisions live in; a stopped review never reaches this stage's writes. Read [references/reporting.md](references/reporting.md) before composing the document or comment.

### Attach the Triage investigation document

Create one issue-scoped Linear document per ticket: `save_document` with `issue` set to the ticket and `title: "Triage investigation"`. For a `Bug` the review in Stage 5 already created it, and the critic approved that exact version. Do not patch it before routing: `engineering-handoff` compares the document Linear holds against the version the critic echoed, and any earlier edit would fail that check. After the handoff has read its writes back (or, for a non-actionable outcome, after the comment is posted), make one `patch` that sets `**Review**: Approved <the updatedAt the critic echoed> at <commit>` and adds whatever routing produced. A stopped review patches immediately, because nothing structural follows it. Passing `issue` is what attaches it, so it appears as a resource on the ticket itself rather than a document filed somewhere else, and anyone reading the ticket can open it in one click. It is the handoff to whoever acts next, and it holds everything the ticket comment leaves out. Where this skill says to record or say something in the report, it means this document, unless it names the comment.
...
Last, after the Triage investigation document is attached and the classification is final, call `record_investigation_case`. A `Bug` is final only when its review was approved; a stopped review records nothing. Not before: a case written from a half-finished investigation is a wrong answer that the next ticket inherits.
