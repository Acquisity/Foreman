...
1. Write the investigation document now. Create or update the issue-scoped `Triage investigation` document exactly as Stage 6 describes, from the template in [references/reporting.md](references/reporting.md), with every Stage 4 lane, the Stage 5 decisions, and the line `**Review**: Pending critic`. Read the document back and keep its id and `updatedAt`; the critic reviews that exact version. When Aaron has asked for read-only validation, write nothing to Linear and instead carry the full document text into the critic message.
...
5. On `APPROVE`: set the document's `**Review**` line to `Approved` with the critic's echoed `document_updated_at` and `commit`, and continue to Stage 6 with the decisions as approved. Do not change the evidence or the decisions after this point; any change reopens the review.
6. On the first `CHALLENGE`: address each blocking finding with its named next check, once. Re-run the evidence lane or correct the decision it points at, update the document, read back its new `updatedAt`, and delegate again with `attempt 2`, the new `updatedAt`, and the list of criteria that failed. Attempt 2 re-judges those criteria against the corrected document. When the correction changed a material decision rather than a detail, say so, because the critic then re-judges all twelve.
