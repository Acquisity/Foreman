...
The final post in the Slack thread is a single-audience message, and the complete final assistant message posts to Slack verbatim. It contains only the forwardable requester reply. Never combine it with an internal investigation summary, a Linear update report, proof of work, or a preamble such as `Here is my investigation summary and Slack reply`. A clarifying question follows the same boundary: that message asks only the question. Normal assistant progress updates may be conversational, but they do not change this final-post boundary.
