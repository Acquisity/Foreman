...
If the parent gave you neither an `uploads.linear.app` url nor a file path, do not guess a path: answer that the image could not be read, leave `visible_text` empty, and put "no url or path was given" in `uncertainties`.
