...
4. Verify lint, typecheck, and tests, all three. Start from the commands the analysis names: its test strategy scopes the checks to the change, so run those and record exactly what you ran and what it produced. For any of the three categories the analysis leaves out, discover the repository's own command in package.json or CI config and run it, scoped to the changed area where the tooling allows it. If something could not be verified, say so explicitly rather than implying it works.
