...
export default defineTool({
  description:
    ...
    "Use pinnedOrganizationId to scope every later PlanetScale query; Autumn and Stripe key on the billing account instead, so pass them billingAccount.id from read_billing_account and its stripe_id. When ambiguous is true the email belongs to several workspaces, so pick the one the report is about or ask the requester. " +
