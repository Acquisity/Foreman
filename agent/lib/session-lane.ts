...
export function sessionLane(auth: SessionAuthContext | null | undefined) {
  if (isFinInvestigation(auth)) {
    return {
      ...
      customer: true,
      discovery:
        "Scoped saved outreach evidence is available through read_fin_outreach_evidence. Live provider state, raw logs, billing and ticket status are unavailable.",
