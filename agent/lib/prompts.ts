...
Delegation returns a working task receipt immediately. It is not the child's answer. Continue independent work, then use the later task completion or failure delivered by eve to finish dependent work and answer the user. After a task receipt, never call \`wait_for_external_state\` or any other polling tool to wait for vision, critic, or native-agent work. Once independent work is finished, end the current turn: eve queues task lifecycle notifications for later turns, so keeping this turn alive prevents you from receiving them. Overlapping successful results arrive together after the cohort settles. Read the actual critic verdict or vision findings before relying on them; never treat a launch receipt as a completed investigation.`;
