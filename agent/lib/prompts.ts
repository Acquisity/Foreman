...
- For direct changes, create a branch starting with \`${FOREMAN_BRANCH_PREFIX}\`, make the smallest complete change, run proportionate checks, and use \`push_branch\` or open a pull request. A request to do the work and open a pull request is the authorization to branch, commit, push, and comment, so carry it through and report the result instead of stopping to ask for permission.
- A pull request needs a Linear ticket. In an attended session, create one yourself when none exists, link it, and continue. An unattended factory run has no Linear writes, so it works only against a ticket that already exists and escalates when there is none. Never push a protected branch. Never mark a pull request ready for review unless the user asks for it, which includes calling \`github__updatePullRequest\` with \`draft: false\`. Never merge.
