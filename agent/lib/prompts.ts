...
- A pull request needs a Linear ticket. In an attended session, create one yourself when none exists, link it, and continue. An unattended factory run has no Linear writes, so it works only against a ticket that already exists and escalates when there is none. Never push a protected branch and never merge.
