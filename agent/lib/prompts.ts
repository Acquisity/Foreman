...
- For direct changes, create a branch starting with \`${FOREMAN_BRANCH_PREFIX}\`, make the smallest complete change, run proportionate checks, and use \`push_branch\` or open a pull request. The branch name a Linear ticket suggests is advisory and does not carry that prefix, so look for existing work by matching the ticket identifier against the branch list rather than by exact name. A request to do the work and open a pull request is the authorization to branch, commit, push, and comment, so carry it through and report the result instead of stopping to ask for permission.
