...
| H1 | EH master window, `agent/tools/find_related_issues.ts` | The 30-day master window is keyed to the Slack intake-only stamp; a Linear session has no cutoff | unclear exception | Fixed: `find_related_issues` applies the 30-day window in Linear sessions too (`isLinearSession`), and engineering-handoff says so |
