...
| `agent/lib/widget-router.ts` | 5s per request | ENG-13999. TypeSafe Jev intent router, composed with the caller's signal. Any failure or a missing `TYPESAFE_API_KEY` returns the `investigate` fallback, so the widget keeps its single pipeline. Sends the customer's latest message whole (4,000 characters accepted) and then up to 7,000 characters of the earlier turns Acquisity supplied, newest first, 2,000 per turn, with cuts and omissions marked; 12,000 in total. |
| `agent/lib/widget-kb.ts` (`decideFromArticles`) | 3s per request | ENG-13999. Jev decides what the read help-center articles can do for the message (answer, not covered, which product, and on an account-likely ask also account or unclear) before any answer is written; the writer runs only on answer. Sends the same conversation format as the router and the read articles (at most 4 at 8,000 characters each). Any failure, a missing `TYPESAFE_API_KEY` or a choice off the menu leaves the call to the writer, as before. |
| `agent/tools/widget_file_ticket.ts` (`isRefundTicket`) | 3s per request | ENG-13999. Before a widget ticket is filed, Jev decides from the ticket's own title and summary whether it asks for a refund; a refund goes to the Support project with the Refund label, assigned to the billing owner. Sends only that title and summary. Any failure or a missing `TYPESAFE_API_KEY` files the ordinary Engineering ticket. |
| `agent/lib/widget-review.ts` | 10s per request | JEV item-level widget review using existing `TYPESAFE_API_KEY`. At most 60 numbered items and 60,000 state characters; no silent truncation. Missing, malformed, uncertain or failed review blocks. Deterministic ownership checks and reply composition are unchanged. |
| `agent/lib/widget-next-action.ts` | 3s per request | ENG-13999 Preview pilot, only when `WIDGET_NEXT_ACTION=jev` and not Production. Jev picks the widget investigator's next action after each tool result, composed with the model call's abort signal. Sends the same conversation format as the router (12,000 characters), this turn's tool calls and results (48,000 characters in total, cuts marked) and the remaining allowlisted tools with their descriptions (1,500 characters each); never model reasoning. The same bounded request asks once more at the finish, only when the findings ask for a person, whether the customer asked for one or billing needs reconciling, sending the conversation and the findings' claims and recommendation; if that fails the findings stand. A timeout, bad status, missing key or answer off the menu leaves the step to the existing budgeted investigation and is logged as a fallback. |
