...
| `agent/lib/widget-router.ts` | 5s per request | ENG-13999. TypeSafe Jev intent router, composed with the caller's signal. Any failure or a missing `TYPESAFE_API_KEY` returns the `investigate` fallback, so the widget keeps its single pipeline. Sends only the customer's latest message, capped at 8,000 characters. |
...
| `agent/lib/widget-next-action.ts` | 3s per request | ENG-13999 Preview pilot, only when `WIDGET_NEXT_ACTION=jev` and not Production. Jev picks the widget investigator's next action after each tool result, composed with the model call's abort signal. Sends the conversation (8,000 characters), this turn's tool calls and results (48,000 characters in total, cuts marked) and the remaining allowlisted tool names; never model reasoning. A timeout, bad status, missing key or answer off the menu leaves the step to the existing budgeted investigation and is logged as a fallback. |
