...
Fin Preview investigations reuse the existing run delivery path. `fin-investigation.ts` subscribes to the session stream for at most 120 seconds (the session continues independently), waits up to eight seconds in a synchronous start, and bounds the final identity recheck to five seconds. `fin-delivery.ts` bounds ownership/history reads to 50 seconds. `fin-investigation-callback.ts` sends only a ready notification to the native Intercom callback with a five-second timeout. `fin-investigation-slack.ts` bounds each Slack HTTP request to five seconds; credential resolution retains the Connect exemption. `fin-run-store.ts` uses the existing deadline-bounded private Postgres connection. No new provider transport is introduced.
