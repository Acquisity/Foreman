...
`agent/lib/fin-preview.ts` starts synthetic or internal diagnostic tasks through Eve's native channel `send` and reads their typed event streams. Session start and stream acquisition expose no authored abort parameter and remain under the hosting function limit. Synthetic observations and diagnostic result lookups are bounded to ten seconds. A diagnostic start uses a bounded 120-second HTTP result observer; the HTTP response waits up to eight seconds after start and may return pending while the explicitly retained background observer continues. Each observer cancels its reader at completion or timeout without cancelling the agent task. Signed run handles permit later replay through native `attachSession`; no new storage or delivery queue is involved. The live test must establish whether startup plus the response fits Intercom's connector deadline.

`agent/lib/fin-preview-slack.ts` posts and updates one task receipt in the configured internal Slack channel, with a five-second deadline on each HTTP request. It uses the existing Preview Slack connector; token resolution has the Connect exemption above. The initial receipt is posted before diagnostic dispatch and its channel/timestamp are persisted in channel state. Native terminal events update the same receipt independently of the HTTP observer. Synthetic probes retain route `waitUntil` delivery. Errors are logged without message bodies and do not block the independent Fin callback. This is best-effort visibility with no scheduled retries; successful updates are marked delivered in channel state.

`agent/lib/fin-preview-callback.ts` sends the completed diagnostic or a fixed failure to Intercom's generated Procedure callback URL with a five-second deadline and redirects disabled. Only the verified HTTPS `api.intercom.io/hooks/procedures/callback/<token>` destination is accepted. Eve's persisted Fin channel state keeps the callback and latest final answer outside the agent prompt; native terminal events deliver after the task ends, independent of the route's observer window. A failed callback is logged without the URL or report and is not retried; the internal Procedure's one-hour timeout assigns the test to Aaron. The signed result lookup remains available.
