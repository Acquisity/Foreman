...
`agent/lib/fin-preview.ts` starts a fixed synthetic task through Eve's native channel `send` and reads its typed event stream. Session start and stream acquisition expose no authored abort parameter and remain under the hosting function limit. Reading the acquired stream is limited to ten seconds and cancels the reader on completion or timeout. A timeout reports `pending`, never success; it does not cancel the agent task. This first proof adds no result polling, delivery queue or customer-data input. The live test must establish whether startup plus the reply fits Intercom's connector deadline.

`agent/lib/fin-preview-slack.ts` posts and updates one synthetic test receipt in the configured internal Slack channel, with a five-second deadline on each HTTP request. It uses the existing Preview Slack connector; token resolution has the Connect exemption above. Eve's route `waitUntil` keeps delivery alive independently of the HTTP response, so Slack does not extend Fin's wait. Errors are logged without message bodies and do not change the probe result. This is best-effort visibility, with no delivery retry or later-result tracking after a pending probe.
