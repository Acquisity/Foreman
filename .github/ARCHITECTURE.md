...
`agent/lib/trust.ts` is the sole caller-trust authority. Unattended runs are denied writes to shared repository knowledge, and global model configuration. For repository knowledge and model configuration, trusted attended callers write directly; other attended callers receive approval prompts. Company services share the same Executor catalog across attended and unattended workflows; workflow instructions govern their use.
...
A literal stop/cancel directly retires the exact durable session through native session reset. Sending reset directly avoids a cancel-then-reset race in which a cancelled child can wake the root. One idempotent "Stop requested." keyed by the retired session ID follows a successful `reset`; a missing session or `no_active_session` stays quiet. Parked existing sessions are retired too. The next Slack message starts a fresh internal session in the same thread with bounded visible history. Hidden notes, tool results and sandbox state are lost. Targeted `task_cancel` remains available for cancelling selected children while retaining the root session.

When an existing Slack thread has no active Eve session, dispatch restores earlier visible messages through the last bot reply using Eve's public history helper. Native lookback supplies the messages after that reply. The added prefix is capped at 32,000 characters, drops whole oldest messages and identifies truncation or unavailable history. The helper requests one page of 50 replies. This is untrusted context, never fresh authorization or restored tool results, internal notes or sandbox files. Version cutover retires old internal sessions after work drains while keeping the Slack threads; the reset and rollback checks are in [UAT-BATTERY.md](./UAT-BATTERY.md).
