...
The request carries the native Intercom conversation id, the customer's question, and an optional Intercom procedure callback URL. Foreman resolves the bearer token through Acquisity's fixed `/api/fin/context` endpoint and persists only that server-verified context in the session initiator. Caller-supplied workspace, organization, user, role, or origin fields are rejected.
