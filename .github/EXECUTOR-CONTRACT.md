...
For example, if a verified customer tool accepts `{path: {customer: string}}`, its binding maps `"path.customer": "path.id"`. This describes argument mapping only; obtain the actual tool path from Executor. A binding is never supplied by the model. Missing, malformed, management, or unknown bindings fail without calling a provider. Query parameters initially retain the request string representation, including arrays for repeated parameters. A verified binding may declare destination-field `coercions`: `number` accepts a finite decimal string, `boolean` accepts only `true` or `false`, and `single` accepts exactly one array item. This preserves the helpers' fixed query values while matching the installed operation schema. Ambiguous values fail before transport.
...
Preview provisioning and live evidence are tracked in [EXECUTOR-PREVIEW.md](./EXECUTOR-PREVIEW.md). Production has not been switched. Run `pnpm executor:readiness` to detect missing bindings/catalog coverage and `EXECUTOR_SETUP_PROFILE=<official-cli-profile> pnpm executor:readiness --live` to audit all fifteen installed default-deny toolkits. This operator command reads configuration with the official CLI's authenticated profile; it never resolves provider credentials or invokes provider tools. A passing configuration audit does not replace the acceptance checklist.
